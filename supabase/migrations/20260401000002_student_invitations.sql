...
-- Indexes
CREATE UNIQUE INDEX IF NOT EXISTS idx_student_invitations_token ON public.student_invitations(token);
CREATE UNIQUE INDEX IF NOT EXISTS idx_student_invitations_pending_student ON public.student_invitations(student_id) WHERE status = 'pending';

-- RLS
ALTER TABLE public.student_invitations ENABLE ROW LEVEL SECURITY;

-- Tutors can view invitations they created
DROP POLICY IF EXISTS "Tutors can view their own invitations" ON public.student_invitations;
...
DROP POLICY IF EXISTS "Tutors can insert invitations" ON public.student_invitations;
...
DROP POLICY IF EXISTS "Tutors can update their own invitations" ON public.student_invitations;
